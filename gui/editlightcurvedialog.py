...
# Form implementation generated from reading ui file 'editlightcurvedialog.ui'
#
...
class Ui_EditLightCurveDialog(object):
    def setupUi(self, EditLightCurveDialog):
        ...
        EditLightCurveDialog.resize(640, 520)
        EditLightCurveDialog.setMinimumSize(QtCore.QSize(640, 520))
        EditLightCurveDialog.setMaximumSize(QtCore.QSize(640, 520))
        ...
        self.noise_combobox = QtGui.QComboBox(EditLightCurveDialog)
    ...
    def retranslateUi(self, EditLightCurveDialog):
        ...
        self.label_16.setToolTip(_translate("EditLightCurveDialog", "Set standard deviation apply method. [?]", None))
        ...
        self.label_17.setToolTip(_translate("EditLightCurveDialog", "Set how observational scatter scales with the light level. [?]", None))
        self.label_17.setWhatsThis(_translate("EditLightCurveDialog", "<html><head/><body><p><span style=\" font-weight:600;\">NOISE</span> should be set to 1 for scatter that scales with the square root of the light level, such as counting statistics, and to 2 for scatter that scales with the light level, such as scintillation noise or fluctuations in sky transparency. If <span style=\" font-weight:600;\">NOISE</span> is set to 0, no level-dependent weighting is applied. </p><p>As a side note, level-dependent weighting does not apply to velocity curves.</p></body></html>", None))
        self.label_17.setText(_translate("EditLightCurveDialog", "NOISE", None))
        self.label_53.setText(_translate("EditLightCurveDialog", "Load or modify light curve parameters:", None))
        self.whatsthis_btn.setText(_translate("EditLightCurveDialog", "?", None))
        self.noise_combobox.setItemText(0, _translate("EditLightCurveDialog", "None", None))
        self.noise_combobox.setItemText(1, _translate("EditLightCurveDialog", "Square Root", None))
        self.noise_combobox.setItemText(2, _translate("EditLightCurveDialog", "Linear", None))
        self.label_18.setToolTip(_translate("EditLightCurveDialog", "Interstellar extinction in magnitude in the designated [LINKEXT] photometric band. [?]", None))
        self.label_18.setWhatsThis(_translate("EditLightCurveDialog", "<html><head/><body><p>AEXTINC refers to a definite photometric band (the band designated by control integer LINKEXT), so it has only one value and it is not band-dependent.</p></body></html>", None))
        self.label_18.setText(_translate("EditLightCurveDialog", "AEXTINC", None))
        self.wla_ipt.setText(_translate("EditLightCurveDialog", "0", None))
        self.aextinc_ipt.setText(_translate("EditLightCurveDialog", "0", None))
        self.label_19.setToolTip(_translate("EditLightCurveDialog", "<html><head/><body><p>Observational wavelenght in microns. </p><p>Wavelengths are no longer used for light curves, which are based on integrated bandpass radiation since the 2003 revision.</p><p>They are still entered for use as reference wavelengths for line profiles and for opacity computations in circumstellar attenuation.</p></body></html>", None))
        self.label_19.setText(_translate("EditLightCurveDialog", "WLA", None))
        self.label_20.setToolTip(_translate("EditLightCurveDialog", "Flux calibration constant in erg s −1 cm −3 for a star of magnitude 0.00.  ", None))
        self.label_20.setText(_translate("EditLightCurveDialog", "CALIB", None))
        self.xunit_ipt.setText(_translate("EditLightCurveDialog", "1.0000", None))
        self.calib_ipt.setText(_translate("EditLightCurveDialog", "0", None))
        self.label_21.setToolTip(_translate("EditLightCurveDialog", "The multiplier that restores the flux numbers to directly inverted magnitudes. [?]", None))
        self.label_21.setWhatsThis(_translate("EditLightCurveDialog", "<html><head/><body><p>This quantity is intended only for flux (as opposed to magnitude) input, and allows rescaling of observed fluxes so that the input flux numbers can be in a convenient range. If input fluxes are simply inverted magnitudes (e.g. 10<span style=\" vertical-align:super;\">−0.4V</span> for V magnitudes), then XUNIT should be unity. However, it may be convenient to enter rescaled (i.e. wrongly scaled) fluxes in cases where they would otherwise be of order, say, 10<span style=\" vertical-align:super;\">−10</span> or 10<span style=\" vertical-align:super;\">3</span> . Then XUNIT can be the multiplier that restores the flux numbers to directly inverted magnitudes. If working in (presumably standard) magnitudes with control integer MAGLITE=1, of course, then don’t worry about XUNIT—just set it to 1.0000.</p></body></html>", None))
        self.label_21.setText(_translate("EditLightCurveDialog", "XUNIT", None))
